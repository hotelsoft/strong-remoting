...
function coerceAccepts(uarg, desc) {
  ...
  if(targetType === 'moment') {
    console.info("bypassing the typeMismatch for moment case", actualType);
    typeMismatch = actualType !== 'string';
  } else {
    typeMismatch = targetType !== 'any' &&
      actualType !== 'undefined' &&
...
function convertValueToTargetType(argName, value, targetType) {
  switch (targetType) {
    ...
    case 'moment':
      return "2016-06-26T10:12:10-10:00"; //moment.tz(value, 'US/Hawaii').format()
